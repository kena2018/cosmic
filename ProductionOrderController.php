    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            ->editColumn('company_name', function ($order) {
                return $order->company ? $order->company->company_name : 'Unknown'; 
            })
            ...
            ->editColumn('sales_order', function ($order) {
                return $order->customerOrder ? $order->customerOrder->order_id : 'Unknown';
            })
    ...
    public function create()
{
    ...
    //         $product->toArray(),
    //         [
    //             'pipe_size' => $product->pipeSizeMaterial->material_name ?? $product->pipe_size,
    //             'outer_name' => $product->outerNameMaterial->material_name ?? $product->outer_name,
    //             'carton_no' => $product->cartonMaterial->material_name ?? $product->carton_no,
    //         ]
    //     );
    // });
    $customers = Customer::orderBy('first_name')->get();
    ...
    return view('admin.production-order.add', compact(
        'products', 'customers', 'laminationpapernames', 'laminationnames', 'laminationgums', 'metaTitle'
    ));
}
...
info($pendingBundleQty);
    ...
    return view('admin.production-order.edit', compact(
        'sticchingPackingType', 'laminationgums', 'laminationnames', 'laminationpapernames',
        'productionOrder', 'products', 'customers', 'salesOrders', 'metaTitle', 'pendingBundleQty'
    ));
}
...
public function getCompaniesByProduct($productId, $salesOrderId = null)
{
    ...
    public function getProductByMaterial($materialId) {
        ...
        return response()->json([
            ...
            'category_name' => $material->category->name ?? '', // Category name
    ...
    public function print( $id ){
        ...
        $productionOrder = ProductionOrder::find($id);
        $salesOrders = CustomerOrder::where('customer_id', $productionOrder->company_name)->get();
        return view('admin.production-order.print', compact('productionOrder','products','customers','salesOrders'));
    }
}
