@extends('layouts.app')
@section('navbarTitel', 'Permission Create')

@section('content')
    <div class="main-outer">
        <div class="outer card">
            <div class="heading-btn">
                <span class="addsupplier-section-heading">Permission Create</span>
                <button type="button"  class="orderList"><span class="back-icons back-tab-icon"></span></button>
            </div>
            <hr class="addsupplier-section-border">
            <form class="needs-validation" action="{{ route('permissions.store')}}" method="post" novalidate id="permissionadd">
                @csrf
                <div class="row permission-inp">
                    <div class="col-lg-4 mb-3">
                        <label class="heading-content"  for="name">Permission Name<span style="color: red;">*</span></label>
                        <input type="text" class="form-control input-form-content @error('name') is-invalid @enderror @if(!empty(old('name'))) is-valid @endif" name="name" id="name" placeholder="Permission Name" value="{{old('name')}}"  >
