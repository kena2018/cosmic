@extends('layouts.app')
@section('navbarTitel', ' Price List Edit')
@section('content')

    <div class="main-outer">
        <div class="outer card">
            <div class="heading-btn">
                <span class="addsupplier-section-heading">Price List Edit</span>
                <button type="button"  class="orderList"><span class="back-icons back-tab-icon"></span></button>
            </div>
            <form  id="newPriceCreate" class="form_mn" action="{{ route('price-list.update', $priceList->id) }}" method="POST">
                    ...
                    <div class="row customer-files-sec">
                        <div class="row form-inp-group">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="list_name" class="form-control input-form-content" id="list_name" value="{{ old('list_name', $priceList->list_name) }}" placeholder="Cosmic product list 16-01-24" >
                            ...
                            <div class="col-md-6 mb-3" >
                                ...
                                <input type="text" name="discount" class="form-control input-form-content" id="discount" value="{{ old('discount',$priceList->discount) }}" placeholder="20%" >
                        ...
                        @foreach($PriceListItems as $index => $PriceListItem)
                            ...
                            <tr class="grouppricing-select-data">
                                <td class="table-data-contents">
                                    <select class="custom-select d-block w-100 form-select-grp select2 product-select" id="product_id_{{$PriceListItem->id}}" name="product_id[]">
                                        <option value="">Select</option>
                                        @foreach($products as $product)
                                        <option value="{{$product->id}}" data-minqty="{{$product->min_quantity}}" data-rate="{{$product->rate}}" data-product="{{$product}}" data-bdl="{{$product->bdl_kg}}" data-bharti="{{$product->bharti}}" data-bagbox="{{$product->number_of_bags_per_box}}"  data-id="{{$product->id}}"{{ old('product.0',$PriceListItem->product_id) == $product->id ? 'selected' : '' }}>{{$product->product_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('product.0')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </td>
                                <td class="table-data-contents">
                                    <select class="custom-select d-block w-100 form-select-grp select2 sku-select" id="sku_id_{{$PriceListItem->id}}" name="sku_id[]">
                                        <option value="">Select</option>
                                        @foreach($products as $product)
                                        <option value="{{$product->id}}" data-minqty="{{$product->min_quantity}}" data-rate="{{$product->rate}}" data-product="{{$product}}" data-bdl="{{$product->bdl_kg}}" data-bharti="{{$product->bharti}}" data-bagbox="{{$product->number_of_bags_per_box}}"  data-id="{{$product->id}}"{{ old('product.0',$PriceListItem->product_id) == $product->id ? 'selected' : '' }}>{{$product->alias_sku}}</option>
                                        @endforeach
                                    </select>
                                    @error('sku.0')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </td>
                                <td class="table-data-contents">
                                    <input type="text" name="min_qty[]" value="{{old('min_qty.0',$PriceListItem->min_qty ?? '')}}"class="order-input @error('min_qty.0') is-invalid @enderror @if(!empty(old('min_qty.0'))) is-valid @endif">
                                    @error('min_qty.0')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </td>
                                <td class="table-data-contents">
                                    <input type="text" name="rate[]" value="{{old('rate.0',$PriceListItem->rate ?? '')}}"class="order-input @error('rate.0') is-invalid @enderror @if(!empty(old('rate.0'))) is-valid @endif">
                                    @error('rate.0')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </td>
                                <td class="table-data-contents">
                                    <input type="text" name="discount_rate[]" id="discount_rate" value="{{old('discount_rate.0',$PriceListItem->discount_rate ?? '')}}"class="order-input @error('discount_rate.0') is-invalid @enderror @if(!empty(old('discount_rate.0'))) is-valid @endif" readonly>
                                    @error('discount_rate.0')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                </td>
                                <td class="table-data-contents">
                                    <input type="text" name="special_rate[]" id="special_rate" value="{{old('special_rate.0',$PriceListItem->special_rate ?? '')}}"class="order-input @error('special_rate.0') is-invalid @enderror @if(!empty(old('special_rate.0'))) is-valid @endif">
        ...
        $(document).ready(function () {
            ...
            $('#discount').on('keydown', function(e) {
                ...
                if (e.key === 'Tab') {
                    ...
                    $('.product-select').select2('focus'); // Move to the first product select field
                }
            });
            ...
            // $('tbody').on('keyup', 'input[name="min_qty[]"]', function() {
            //     var row = $(this).closest('tr');
            //     var selectedProduct = row.find('.product-select option:selected');
            //     var min_quantity = selectedProduct.data('minqty');
            //     var enteredQty = $(this).val();
            //     row.find('.error-message').remove();
            //     if (parseInt(enteredQty) > min_quantity) {
            //         // Display error message
            //         var errorMessage = $('<div class="invalid-feedback error-message" style="display: block;">Quantity cannot exceed ' + min_quantity + '</div>');
            //         // Add the error message after the min_qty input field
            //         $(this).after(errorMessage);
            //         // Add Bootstrap invalid class to the input
            //         $(this).addClass('is-invalid');
            //         // Reset the input to the minimum quantity
            //         $(this).val(min_quantity);
            //     } else {
            //         // If valid, remove invalid class and any error messages
            ...
            $('tbody').on('keyup', 'input[name="special_rate[]"]', function() {
                ...
                // If a special rate is entered, hide the discount_rate field
                if (enteredQty.trim() !== '') {
                    // row.find('input[name="discount_rate[]"]').val('');
                    // row.find('input[name="discount_rate[]"]').closest('td').hide(); // Hide the discount_rate field
                } else {
                    // If special rate is cleared, show the discount_rate field again
                    // row.find('input[name="discount_rate[]"]').closest('td').show();
                }
            });
