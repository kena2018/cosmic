...
<html>
<head>
<title>Customer Order PDF</title>
<style>
    *{font-family: sans-serif;}
    ...
    .header-table .right-section img {  max-width: 90px; margin-left: -4px;font-size: 25px;}
    ...
    .section-heading {font-size: 17px; font-weight: 600; color: #6B4023; padding: 15px 0 10px 3px;}
    ...
    .last-section {margin: 20px 0 8px 0; text-align: left;}
...
<div class="pdf-container">
    ...
    <div class="data-container">
        <table class="header-table">
            <tr>
                ...
                <td class="project_data">
                    <div class="project-title">Customer Order</div>
        ...
        <div class="section-heading first-section-heading">Order Information</div>
        <table class="info-table first-tablee">
            <tr>
                <th class="table-heading-sec">Company:</th>
                <td>{{ old('customer_name', $customerOrders->customer->company_name ?? '') }}</td>
            </tr>    
            <tr>
                <th class="table-heading-sec">Phone no.:</th>
                <td>{{old('contact',$customerOrders->contact ?? '')}}</td>
            </tr>
            <tr>
                <th class="table-heading-sec">Price List:</th> 
                <td>{{ optional($pricelists->find($customerOrders->price_list))->list_name }}</td>
            </tr>
            <tr>
                <th class="table-heading-sec">Delivery Date:</th>
                <td>{{ old('delivery_date', isset($customerOrders->delivery_date) ? \Carbon\Carbon::parse($customerOrders->delivery_date)->format('d-m-Y') : 'N/A') }}</td>
            </tr>
            <tr>
                <th class="table-heading-sec">Delivery Instructions:</th>
                <td>{{ $customerOrders->shipping_address }}</td>
            </tr>
            <tr>
                <th class="table-heading-sec">Transport Details:</th>
                <td>{{ optional($transforms->find($customerOrders->packing_name))->name }}</td>
                
            </tr>
            <tr>
                <th class="table-heading-sec">Order ID:</th>
                <td>{{old('order_id',$customerOrders->order_id ?? '')}}</td>
            </tr>
        </table>

        <div class="section-heading">Products Order Information</div>
        <table class="info-table first-tablee">
        @foreach($productsOrders as $index => $productId)
        <tr>
            <th class="table-heading-sec">Product Name: </th>
            <td>{{ $products->firstWhere('id', $productId->product_id)->product_name ?? '' }}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">SKU:</th>
            <td>{{ $products->firstWhere('id', $productId->product_id)->alias_sku ?? '' }}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Colour:</th>
            <td>{{ old('color.0', $productId->colour ?? '') }}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Sticker:</th>
            <td>{{old('sticker_name.0',$productId->sticker_name ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Packing Material Name:</th>
            <td>{{old('packing_material_type.0',$productId->packing_material_type ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">BDL:</th>
            <td>{{old('bdl_kg.0',$productId->bdl_kg ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Bharti:</th>
            <td>{{old('bharti.0',$productId->bharti ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Bag/Box:</th>
            <td>{{old('unit_box.0',$productId->unit_box ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Total Roll:</th>
            <td>{{old('roll_counte.0',$productId->roll_counte ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Rate:</th>
            <td>{{old('rate.0',$productId->rate ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Total:</th>
            <td>{{old('total.0',$productId->total ?? '')}}</td>
        </tr>
        <tr>
            <th class="table-heading-sec">Remark:</th>
            <td>{{old('remark.0',$productId->remark ?? '')}}</td>
        </tr>
        
        @endforeach
        <tr>
            <th class="table-heading-sec">Customer Notes:</th>
            <td>{{ old('customer_notes', $customerOrders->customer_notes ?? '') }}</td>
        </tr>
        </table>
