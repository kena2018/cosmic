    ...
    public function getLaminationOrdersByStatus(Request $request)
{
    ...
    $laminationOrders = LaminationProductionOrder::select(
        ...
        'production_order.bundle_quantity as production_qty',
        'production_order.pending_bundle_qty',
        'production_order.lamination_internal_notes',
        'product_orders.colour as color',
        'products.alias_sku',
        'products.length',
        'products.width',
    ...
    ->where('lamination_production_orders.status', $status)
    ->whereIn('products.category', ['1', '2'])
    ->orderBy('lamination_production_orders.updated_at', 'desc')
    ->get();
    ...
    $laminationOrders = $laminationOrders->map(function ($order) {
        ...
        $order->meter = number_format($meter);
    ...
    public function getExtruderOrdersByStatus(Request $request)
    {
        ...
        $extruderOrders = ExtruderProductionOrder::select(
            ...
            'production_order.bundle_quantity as production_qty',
            'production_order.pending_bundle_qty',
            'production_order.extrusion_recipe',
            'recipe.recipe_name',
            ...
            'customer_orders.total_bundle as total_order_qty',
            'production_order.extrusion_internal_notes'
        )
        ...
        ->leftJoin('recipe_masters as recipe', 'production_order.extrusion_recipe', '=', 'recipe.id')
        ->leftJoin('materials', 'products.packing_material_type', '=', 'materials.id')
        ->where('extruder_production_orders.status', $status)
        ->orderBy('extruder_production_orders.updated_at', 'desc')
        ->get();
    
        // Fetch Recipe Materials separately and attach them
        $extruderOrders->each(function ($order) {
            if ($order->extrusion_recipe) {
                $order->recipe_materials = RecipeMaterial::where('recipe_master_id', $order->extrusion_recipe)
                    ->get()
                    ->map(function ($material) {
                        return [
                            'id' => $material->id,
                            'recipe_master_id' => $material->recipe_master_id,
                            'material_id' => $material->material_id,
                            'material_name' => Material::where('id', $material->material_id)->value('material_name'),
                            'submaterial_id' => $material->submaterial_id,
                            'submaterial_name' => MaterialSubCategory::where('id', $material->submaterial_id)->value('sub_cat_name'),
                            'up' => $material->up,
                            'downs' => $material->downs,
    ...
    public function getRewindingOrdersByStatus(Request $request)
{
    ...
    $rewindingOrders = RewindingProductionOrder::select(
        ...
        'rewinding_production_orders.status',
        'customer_orders.total_bundle as total_order_qty',
        'production_order.rewinding_internal_notes',
    )
        ->leftJoin('extruder_production_orders', 'rewinding_production_orders.extruder_id', '=', 'extruder_production_orders.id')
        ->leftJoin('products', 'rewinding_production_orders.product_id', '=', 'products.id')
        ->leftJoin('production_order', 'rewinding_production_orders.production_order_id', '=', 'production_order.id')
        ...
        ->where('rewinding_production_orders.status', $status)
        ->orderBy('rewinding_production_orders.updated_at', 'desc')
        ->get();
    ...
    public function getStitchingOrdersByStatus(Request $request)
    {
        ...
        $stitchingOrders = stitchingProductionOrder::select(
            ...
            'production_order.stitching_internal_notes',    
            // 'customer_orders.status'
        )
        ...
        ->orderBy('stitching_production_orders.updated_at', 'desc')
        ->get();
    ...
    public function getPackingOrdersByStatus(Request $request)
    {
        ...
        $packingOrders = PackingProductionOrder::select(
            ...
            'products.alias_sku',
            'materials.material_name',
            'outer_material.material_name as outer_name',
            'packing_sticker.material_name as sticker_name',
            'packing_production_orders.status',
            'production_order.packing_internal_notes',
        ...
        ->leftJoin('materials as outer_material', 'products.outer_name', '=', 'outer_material.id')
        ->leftJoin('materials as packing_sticker', 'production_order.packing_sticker', '=', 'packing_sticker.id')
        ->leftJoin('customer_orders', 'packing_production_orders.customer_order_id', '=', 'customer_orders.id')
        ->leftJoin('materials', 'products.packing_material_name', '=', 'materials.id')
        ->where('packing_production_orders.status', $status)
        ->orderBy('packing_production_orders.updated_at', 'desc')
        ->get();
...
public function setExtruderOrderCompleted(Request $request)
{
   ...
   // if ($thisOrdersCompletedQuantity < $productionOrder->bundle_quantity) {
    ...
    return response()->json([
        'status' => 1,
        'requestCode' => 200,
        'message' => 'Data saved to extruder order history ',
        'required_qty' => $productionOrder->bundle_quantity,
        //'previousCompletedQuantity' => $previousCompletedQuantity,
        'remaining' => $remainingQty - $currentQty,
        //'thisOrdersCompletedQuantity' => $thisOrdersCompletedQuantity
...
public function extruderOrderMakeItComplete(Request $request)
{
    $request->validate([
        'extruder_production_order_id' => 'required|integer|exists:extruder_production_orders,id',
    ]);
    ...
    public function setLaminationOrderCompleted(Request $request)
    {
        ...
        $product = Product::find($laminationOrder->product_id);
        // $bharti = $productionOrder->rewinding_bharti;
        // $length = $productionOrder->rewinding_length;
        // $bundles = $productionOrder->bundle_quantity;
        ...
        $rolls_in_1_bdl = $product->rolls_in_1_bdl;
        $bundles = $productionOrder->bundle_quantity;
    
        // Formula for required meters: bharti * length * bundles
        $fullMeter = $length * $rolls_in_1_bdl * $bundles;
        $required_meters = $fullMeter;
        if (in_array($productionOrder->lamination_type, ['Cutter', 'Strip'])) {
            $required_meters /= 2; // Divide by 2 for Cutter or Strip
        }
      ...
      //  }
        ...
        return response()->json([
            'status' => 1,
            'requestCode' => 200,
            'message' => 'Data saved to lamination order history',
            'required_meters' => $required_meters,
            'thisOrdersCompletedQuantity' => $thisOrdersCompletedQuantity,
    ...
    public function laminationOrderMakeItComplete(Request $request)
    {
        $request->validate([
            'lamination_production_order_id' => 'required|integer|exists:lamination_production_orders,id',
        ]);
    ...
    public function laminationOrderHistoryDelete(Request $request)
    {
        $request->validate([
            'id' => 'required|integer|exists:lamination_order_history,id'
        ]);
    
        $order = LaminationOrderHistory::find($request->input('id'));
    ...
    public function extruderOrderHistoryDelete(Request $request)
    {
        $request->validate([
            'id' => 'required|integer|exists:extruder_order_history,id'
        ]);
    
        $order = ExtruderOrderHistory::find($request->input('id'));
    ...
    public function rewindingOrderHistoryDelete(Request $request)
    {
        $request->validate([
            'id' => 'required|integer|exists:rewinding_order_history,id'
        ]);
    
        $order = RewindingOrderHistory::find($request->input('id'));
    ...
    public function packingOrderHistoryDelete(Request $request)
    {
        $request->validate([
            'id' => 'required|integer|exists:packing_order_histories,id'
        ]);
    
        $order = PackingOrderHistory::find($request->input('id'));
    ...
    public function stitchingOrderHistoryDelete(Request $request)
    {
        $request->validate([
            'id' => 'required|integer|exists:stitching_order_histories,id'
        ]);
    
        $order = StitchingOrderHistory::find($request->input('id'));
    ...
    public function setRewindingOrderCompleted(Request $request)
    {
        ...
        $remainingQty = $productionOrder->rewinding_qty_in_rolls - $previousCompletedQuantity;
        ...
        return response()->json([
            ...
            'requestCode' => 200,
            'message' => 'Data saved to rewinding order history due to insufficient bundle quantity',
            'required_qty' => $productionOrder->rewinding_qty_in_rolls,
            'remaining' => $remainingQty - $currentQty,
        ]);
    }

    public function rewindingOrderMakeItComplete(Request $request)
{
    $request->validate([
        'rewinding_production_order_id' => 'required|integer|exists:rewinding_production_orders,id',
    ]);
    ...
    public function setPackingOrderCompleted(Request $request)
    {
       ...
       // $packingOrder->status = 'completed';
        ...
        return response()->json([
            'status' => 1,
            'requestCode' => 200,
            'message' => 'Data saved to Packing order history.',
            'required_qty' => $productionOrder->bundle_quantity,
            'remaining' => $remainingQty - $currentQty,
        ]);
    ...
    public function packingOrderMakeItComplete(Request $request)
{
    $request->validate([
        'packing_production_order_id' => 'required|integer|exists:packing_production_orders,id',
    ]);
    ...
    //     ]);
        return response()->json([
            'status' => 1,
            'requestCode' => 200,
            'message' => 'Data saved to stitching order history.',
            'required_qty' => $productionOrder->bundle_quantity,
            'remaining' => $remainingQty - $currentQty,
        ]);
    }

    public function stitchingOrderMakeItComplete(Request $request)
{
    $request->validate([
        'stitching_production_order_id' => 'required|integer|exists:stitching_production_orders,id',
    ]);
    ...
    public function getLaminationOrdersSearch(Request $request)
{
    ...
    if ($search) {
        $query->where(function($q) use ($search) {
            $q->where('customer_orders.order_id', 'like', '%' . $search . '%')
              ->orWhere('customer_orders.serial_number', 'like', '%' . $search . '%')
              ->orWhere('products.product_name', 'like', '%' . $search . '%')
              ->orWhere('production_order.lamination_type', 'like', '%' . $search . '%')
              ->orWhere('lamination_material.material_name', 'like', '%' . $search . '%')
              ->orWhere('lamination_film.material_name', 'like', '%' . $search . '%')
              ->orWhere('lamination_production_orders.machine', 'like', '%' . $search . '%')
              ->orWhere('lamination_production_orders.status', 'like', '%' . $search . '%');
        });
    }
    ...
    // Apply the LIKE query on the calculated fields (meter, paper_kg, and film_kg)
    // if ($search) {
    //     $orders = $orders->filter(function ($order) use ($search) {
    //         return stripos($order->order_id, $search) !== false ||
    //                stripos($order->serial_number, $search) !== false ||
    //                stripos($order->product_name, $search) !== false ||
    //                stripos($order->lamination_type, $search) !== false ||
    //                stripos($order->lamination_material_name, $search) !== false ||
    //                stripos($order->film_name, $search) !== false ||
    //                stripos($order->machine, $search) !== false ||
    //                stripos($order->status, $search) !== false ||
    //                stripos($order->meter, $search) !== false ||
    //                stripos($order->paper_kg, $search) !== false ||
    //                stripos($order->film_kg, $search) !== false;
    //     });
    // }

    return response()->json($orders);
}
public function getExtruderOrdersSearch(Request $request)
{
    $search = $request->input('search');
    ...
    if ($search) {
        $query->where(function($q) use ($search) {
            $q->where('customer_orders.order_id', 'like', '%' . $search . '%')
              ->orWhere('customer_orders.serial_number', 'like', '%' . $search . '%')
              ->orWhere('products.product_name', 'like', '%' . $search . '%')
              ->orWhere('extruder_production_orders.created_at', 'like', '%' . $search . '%')
              ->orWhere('products.length', 'like', '%' . $search . '%')
              ->orWhere('products.width', 'like', '%' . $search . '%')
              ->orWhere('products.gage', 'like', '%' . $search . '%')
              ->orWhere('product_orders.colour', 'like', '%' . $search . '%')
              ->orWhere('extruder_production_orders.status', 'like', '%' . $search . '%');
        });
    }
    ...
    // Attach `recipe_materials` to each order
    $orders->each(function ($order) {
        if ($order->extrusion_recipe) {
            $order->recipe_materials = RecipeMaterial::where('recipe_master_id', $order->extrusion_recipe)
                ->get()
    ...
    public function getRewindingOrdersSearch(Request $request)
    {
        $search = $request->input('search');
            ...
            if ($search) {
                $query->where(function($q) use ($search) {
                    $q->where('customer_orders.order_id', 'like', '%' . $search . '%')
                      ...
                      ->orWhere('products.length', 'like', '%' . $search . '%')
                      ->orWhere('products.Width', 'like', '%' . $search . '%')
                      ->orWhere('production_order.pending_bundle_qty', 'like', '%' . $search . '%')
                      ->orWhere('products.pipe_size', 'like', '%' . $search . '%')
                      ->orWhere('sticker_material.material_name', 'like', '%' . $search . '%')
                      ->orWhere('rewinding_production_orders.created_at', 'like', '%' . $search . '%');
                });
            }
            $orders = $query->get();
    ...
    public function getPackingOrdersSearch(Request $request)
    {
        $search = $request->input('search');
